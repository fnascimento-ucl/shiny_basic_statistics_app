app_overview_ui <- function() {
  tabPanel("App Overview",
    HTML(paste0(
      ...
      This generated DataTable is dynamic, allowing users to select, modify and filter data that can then be plotted below the table display. After data curation, the DataTable can be save into .xlsx or .cvs format for further processing </p>",
      ...
      Therefore, data dependencies need to be considered.</p/>",
      ...
      This tab also generates a customizable plot with data grouped according to the hierarchical levels and color-coded by grouping variable, as well as a summary table with all the calculated statistics. </p>",
      ...
      "<p style='margin-bottom: 5px;'> The <b>Superplot</b> tab allows the user to generate a <a href='https://rupress.org/jcb/article/219/6/e202001064/151717/SuperPlots-Communicating-reproducibility-and' target='_blank'>superplot plot</a>. This is scatter plot ideal for data with a 2 level structure, that displays superimposed information from all datapoints, median/median per experimental level/replica and summary statistics (standard deviation or interquartile range).</p>",
      "<p style='margin-bottom: 5px;'> The <b> Box, Bar, Scatter, Violin and Raincloud </b> tab provides the multiple plotting style options that are also available in the other sections of this app.</p>",
      "<p style='margin-bottom: 0px;'> The left panel allows the user to choose between several options for estimation statistics. Simple (1 level) or hierarchical bootstrap (up to 4 levels) is used for effect size calculation where a control group is compared against one or multiple other test groups. 
      Mean or median difference provides a measure of absolute unitary changes between groups, whereas Hedges g', Cohen's d and Glass' delta provide effect size coefficients that can be referred to as small (0.20), medium (0.50) and large (0.80).</p>",
      ...
      "<p style='margin-bottom: 0px;'> Allen EA, Erhardt EB, Calhoun VD. Data visualization in the neurosciences: overcoming the curse of dimensionality. Neuron. 2012 May 24;74(4):603-8. doi: 10.1016/j.neuron.2012.05.001. PMID: 22632718; PMCID: PMC4427844 </p>",
